...
### Apply
```bash
git pull --ff-only /path/to/biltoo-1619-fix-controller-access.bundle HEAD
```
...
### Apply
```bash
git pull --ff-only /path/to/biltoo-1611-fix-thumbnailbar-surfaces.bundle HEAD
```
...
### Commits
1. **ImageView::cropPolygonView** shared by overlay and hit-test
2. **PlacementLinear::scenePosDeltaToAlign** for centre alignment

### Apply
```bash
git pull /path/to/biltoo-1498-crop-view-align.bundle HEAD
```
Requires tip **1497** (or **1479** + full stack).
...
### Commits
1. **CropSession** sourceSearchRectFromDraft + seedRotationFromStashedPlacement
2. **CropSession** restoreEnterPlacementPose

### Apply
```bash
git pull /path/to/biltoo-1487-crop-session-more.bundle HEAD
```
Requires tip **1486** (or **1479** + full stack).
...
### Change
1. `GalleryPackFit::modeFromLayoutMode`
2. `GalleryController::clearChrome`
3. `SessionAppearanceStore::take`
4. `HudFlash::setIdentityPulse` reports change

### Apply
```bash
git pull /path/to/biltoo-1389-mode-map-gallery-chrome-appearance-take-hudpulse.bundle HEAD
```
Requires upstream **3830dda** (tips 1376–1389).
...
- `HudGeometry::{maxPanelBgW,maxPanelTextW}`
...
### Apply
```bash
git pull --rebase /path/to/biltoo-1158-gallery-no-soft-job.bundle HEAD
```
...
### Change
- ENVIRONMENT: PreferCache/filmstrip path documented against Store-only thumtoo
- No code change — `scheduleSoftPixels` already PreferCache-when-tiles

### Apply
```bash
git pull /path/to/biltoo-1012-prefercache-store-docs.bundle HEAD
```
...
### Change
No remaining call sites after install/rematerialize consolidation. The multi-MP
branch was a host-under-want footgun (orient-only applied). Callers use
installDisplayPixels / rematerializeItemContent / materializeDisplay /
applyContentToImage only.

### Apply
```bash
git pull /path/to/biltoo-933-remove-dead-applyContentToItem.bundle HEAD
```
...
### Change
- Crop enter: `resolveCropEnterAppearance`, `installFullImageForCrop`, `initCropRectFromPriorAppearance`
- Page guide: `pageGuideRectFromHandleDrag` (pure rect math); `updatePageGuideResize` applies

### Done criteria
- [x] Crop-enter and page-guide resize phases named
- [x] Bundle **626**
...
- thumtoo: interest cancel + FastScale Q1 ≤1024
