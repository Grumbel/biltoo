...
void CropController::recordSessionCrop(ImageItem *item, const QRectF &localCrop)
{
    ...
    const ItemComponents::Placement cropPl = item->placement();
    CropSession::RecordGeometry rec = session().computeRecordGeometry(
        ...
        cropPl.hFlip, cropPl.vFlip);
    if (!rec.valid()) {
        return;
    }
    const SessionImageId sid = cropRecordSessionId(item);
    WorkspaceItemState orientStore;
    const WorkspaceItemState *orientApp = nullptr;
    if (sid != kInvalidSessionImageId) {
        orientStore = m_view->sessionAppearanceValue(sid);
        orientApp = &orientStore;
    }
    ...
    s.path = item->path();
    m_view->syncLiveContentMetaFromState(item, s);
...
bool CropController::prepareCropModeFullImage(ImageItem *item)
{
    ...
    if (CropSession::canKeepDisplayForEnter(
        ...
        CropDebug::draftEnterDone(item->imageSize().width(), item->imageSize().height(),
                                  ...
                                  m_view->itemAppliedContentXform(item).hasCrop, contentOnly.contentQuarterTurns);
        session().markShowingFullImage();
    }
