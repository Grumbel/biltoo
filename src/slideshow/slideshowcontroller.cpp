...
void SlideshowController::paintLetterboxComposite(QPainter &painter) const
{
    ...
    auto fillPad = [&](const QRect &vr, const QImage &fromSrc = QImage(),
        ...
        const QString fPath = !fromPath.isEmpty() ? fromPath
            : (phase().hasFromPath() ? phase().fromPathRef() : dwell().biasPathRef());
        const QString tPath = !toPath.isEmpty() ? toPath : phase().toPathRef();
    ...
    if (hud().isProgressActive()
        && (phase().hasFromImage() || dwell().hasSourceImage() || phase().hasToImage())) {
        ...
        const QImage &fromImg = phase().hasFromImage() ? phase().fromImageRef() : dwell().sourceImageRef();
        const qreal fromT = phase().fromMotionTValue();
        const qreal toT = phase().toMotionTValue();
        if (phase().inTransition() && phase().hasToImage()) {
            const qreal t = phase().clampedFadeT();
            fillPad(vr, fromImg, phase().toImageRef(), t, phase().fromPathRef(), phase().toPathRef());
            if (settings().isFadeBlack()) {
                ...
                if (t < 0.5) {
                    ...
                    painter.setOpacity(1.0);
                    paintMotionCover(&painter, phase().toImageRef(), toT,
                                     phase().toBiasAPoint(), phase().toBiasBPoint(), phase().toPathRef());
                    painter.setOpacity((1.0 - t) * 2.0);
                    painter.fillRect(vr, Qt::black);
                    painter.setOpacity(1.0);
                }
            } else if (settings().isSlideTransition()) {
            ...
            } else if (settings().isNoneTransition()) {
            ...
            fillPad(vr, fromImg, QImage(), -1.0, phase().fromPathRef());
            paintMotionCover(&painter, fromImg, fromT,
                             dwell().biasAPoint(), dwell().biasBPoint(), phase().fromPathRef());
        }
...
void SlideshowController::paintSeekbar(QPainter &painter) const
{
    ...
    if (hud().isProgressActive()
        && (m_view->hostHudPrefs().isVisible() || hud().isSeekbarVisible() || hud().isSeekDragging())) {
        const int viewW = m_view->viewport()->width();
        const int viewH = m_view->viewport()->height();
        if (viewW > 0 && viewH > 0) {
            qreal fraction = 0.0;
            if (hud().timelineTotal() > 0) {
                fraction = qreal(hud().timelineElapsed())
                    / qreal(hud().timelineTotal());
            } else if (hud().isCycleProgressValid()) {
                fraction = hud().cycleProgress();
            } else if (hud().hasProgressInterval()) {
                qint64 elapsed = hud().progressBase();
                if (!hud().isProgressClockPaused()
                    && hud().isProgressElapsedValid()) {
                    elapsed += hud().progressElapsedMs();
                }
                fraction = qreal(elapsed) / qreal(hud().progressInterval());
            }
            fraction = ViewTransform::clamp01(fraction);

            QColor c = m_view->hostHudPrefs().effectiveTextColor(QColor(255, 255, 255));
            QColor track = c;
            track.setAlpha(60);
            c.setAlpha(200);
            ...
            const int barH = HudGeometry::progressBarHeight(hud().isSeekbarVisible());
            ...
            if (hud().timelineTotal() > 0) {
                const qint64 remain = hud().timelineTotal()
                    - hud().timelineElapsed();
                const QString timeLine =
                    QStringLiteral("%1 / %2   −%3")
                        .arg(SlideshowClocks::formatClockMs(hud().timelineElapsed()),
                             SlideshowClocks::formatClockMs(hud().timelineTotal()),
                             SlideshowClocks::formatClockMs(remain));

                QFont f = painter.font();
                f.setPointSize(m_view->hostHudPrefs().effectiveFontPointSize());
                painter.setFont(f);
