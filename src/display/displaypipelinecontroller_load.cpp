...
void DisplayPipelineController::finishLoadAddStatus(bool refreshGalleryWindow)
{
    ...
    if (refreshGalleryWindow && m_host->isGalleryMode()) {
        m_host->hostGallery().scheduleDecodeWindowRefresh(GalleryDecode::kDecodeWindowSettleMs);
    }
}
...
void DisplayPipelineController::handleLoadAddDecodeFailure(const QString &path)
{
    ...
    if (m_host->isGalleryMode()) {
        GalleryDecodeState &st = m_host->hostGalleryDecodeBook().state(path);
        st.failed = true;
        st.inflight = 0;
    }
...
{
    for (ImageItem *cand : m_host->hostGallery().stashedItems()) {
...
void DisplayPipelineController::reassertPendingBindPlacement(const QString &path)
{
    ...
    for (ImageItem *item : m_host->liveItems()) {
        ...
        for (int bi = 0; bi < m_host->hostBindBook().bindCount(); ++bi) {
            const PendingSessionBind &b = m_host->hostBindBook().bindAt(bi);
            if (b.path != path) {
                continue;
            }
...
void DisplayPipelineController::claimUnboundItemsForPendingBinds(const QString &path, const QImage &image)
{
    ...
    for (ImageItem *existing : m_host->liveItems()) {
        ...
        if (!m_host->hostBindBook().takeBind(path, &bound)) {
            break;
        }
        if (bound.id != kInvalidSessionImageId) {
            m_host->setItemSessionId(existing, bound.id);
            if (m_host->sessionListIndex(existing) < 0 && bound.index >= 0) {
...
void DisplayPipelineController::completeLoadAdd(const QString &path, const QImage &image, quint64 generation)
{
    ...
    const int pendingBinds = m_host->hostBindBook().countBindsForPath(path);
...
void DisplayPipelineController::scheduleImageLoad(const QString &path, int role)
{
    ...
    if (role == ImageView::LoadReplace) {
        ...
        m_host->hostGalleryDecodeBook().clearImageModeNativeDecode();
    ...
    if (role == ImageView::LoadReplace && m_host->isImageMode() && m_host->hostSlideshow().hud().isProgressActive()) {
    ...
    if (role == ImageView::LoadReplace && m_host->isImageMode()) {
...
{
    const QImage ready = m_host->hostSlideshow().slideshowRaster(path);
    if (ready.isNull()) {
        return false;
    }
...
void DisplayPipelineController::scheduleClassicImageDecode(const QString &path, quint64 gen, int role)
{
    ...
    if (m_host->isGalleryMode() || m_host->isWorkspaceMode()) {
        ThumtooCache::scheduleProbe(path);
        const QImage cached = ImageCache::get(path);
        if (!cached.isNull()
...
{
    m_host->hostGalleryDecodeBook().resetPath(path);
    if (m_host->hostPathRaster() && !path.isEmpty()) {
        m_host->hostPathRaster()->cancel(path);
    }
}
...
{
    m_host->hostGalleryDecodeBook().clearDecodeStates();
}
...
bool DisplayPipelineController::scheduleGalleryDecode(const QString &path)
{
    ...
    if (anyTileWanted && !m_host->hostGallerySizeResolve().active()) {
        // Size must be known for tile requests.
        if (!m_host->hostSizeBook().hasDefinitive(path)
...
void DisplayPipelineController::completeLoadRestore(const QString &path, const QImage &image)
{
    ...
    if (state.sessionId != kInvalidSessionImageId) {
        m_host->setItemSessionId(item, state.sessionId);
        if (m_host->itemWorld().hasDurableAppearance(state.sessionId)) {
            app = m_host->sessionAppearanceValue(state.sessionId);
    ...
    // bake crop on the GUI and used to claim applied == want without pixels.
    {
        ...
        if (sid != kInvalidSessionImageId) {
            app.colorAdjust = m_host->itemWorld().color(sid).grade;
        }
        if (SessionAppearance::hasContentAppearance(app)
            || !app.colorAdjust.isIdentity()
            || (sid != kInvalidSessionImageId && m_host->itemWorld().hasColor(sid))) {
