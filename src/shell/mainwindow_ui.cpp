...
void MainWindow::createActions()
{
    ...
    m_zoom1to1Act->setCheckable(true);
    m_zoom1to1Act->setIcon(themeIcon(QStringLiteral("zoom-original"), QStyle::SP_DesktopIcon));
    m_zoom1to1Act->setStatusTip(
        tr("1:1 pixels (sticky while checked; click again or zoom ± to release)"));
...
void MainWindow::populateActionHelpTexts()
{
    ...
    setHelp(m_helpGuideWorkspaceAct, tr(
        ...
        "<ul>"
        "<li><b>Place</b> session images on the canvas by <b>dragging</b> from the "
        "filmstrip (or dropping files). Double-clicking a thumbnail opens Image mode; "
        "it does not add to the Workspace. Membership is optional — the session can "
        "hold images that are not on the canvas.</li>"
